...
class NewFakesDataset(Dataset):
    ...
    def __init__(self, h5_paths, x_dim, y_dim, z_dim, start=0, limit=-1):
        ...
        x[:, :10] = [i if i != 0 else np.random.normal(-1, 0.1) for i in x[:, :10].flatten()].reshape(-1, 10)
        x[:, 10:20] = [i if i != 0 else np.random.normal(-7, 0.1) for i in x[:, 10:20].flatten()].reshape(-1, 10)
        x[:, 20:30] = [i if i != 0 else np.random.normal(-7, 0.1) for i in x[:, 20:30].flatten()].reshape(-1, 10)
