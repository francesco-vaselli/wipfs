void f_MJRElectron_plot() {

	auto f_MGenPart = 0.431386;

	TGraph *g = new TGraph("f_MJRElectron.txt");
	...
	auto sGenPart = Form("RECO/GenPart matching fraction: %.2f", f_MGenPart);

	pt1->AddText(sGenPart);
	pt1->Draw();

	auto pt2 = new TPaveText(.15, .7, .4, .88);
	...
	full_match->SetParameter(0, f_MGenPart);
	...
	c->Update();
	c->SaveAs("figures/f_MJRElectron_vs_DR.pdf");
}
