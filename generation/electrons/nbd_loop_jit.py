...
if __name__ == "__main__":
    ...
    new_root = "/gpfs/ddn/cms/user/cattafe/TTJets/jit/"
    ...
    ele_flow, _, _, _, trh, tsh = load_model(
        ...
        filename="E1/checkpoint-latest.pt",
    )  # to be changed
    ...
    wrapped_model = model_from_flow(ele_flow)

    with torch.no_grad():
        fake_input = torch.rand((1, 42)).to(device)
        pred = wrapped_model(x=fake_input)  # Compute caches
    ...
    for path in tqdm(files_paths):
        ...
        nbd_func.nbd(traced_model, root, path_str, new_root)
