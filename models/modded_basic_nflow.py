...
from modded_MADE_mask import NMaskedMADE, CMaskedMADE
...
class MaskedAffineAutoregressiveTransformM(AutoregressiveTransform):
    def __init__(
        ...
        made = CMaskedMADE( # made_module.MADE(
            features=features,
...
class MaskedPiecewiseRationalQuadraticAutoregressiveTransformM(AutoregressiveTransform):
    def __init__(
        ...
        autoregressive_net = CMaskedMADE( # made_module.MADE(
            features=features,
            hidden_features=hidden_features,
            context_features=context_features,
            num_blocks=num_blocks,
...
class MaskedPiecewiseRationalQuadraticAutoregressiveTransformMa(AutoregressiveTransform):
    def __init__(
        self,
        features,
        hidden_features,
...
def create_mixture_flow_model(
    ...
    for i in range(base_kwargs["num_steps_arqs"]):
        if i != base_kwargs["num_steps_arqs"] - 1:
            transform.append(
            ...
            print("last layer")
            transform.append(
                MaskedPiecewiseRationalQuadraticAutoregressiveTransformMa(
                    features=input_dim,
                    tails="linear",
                    use_residual_blocks=base_kwargs["use_residual_blocks_arqs"],
                    hidden_features=base_kwargs["hidden_dim_arqs"],
