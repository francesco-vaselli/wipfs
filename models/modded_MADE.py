"""Implementation of MADE. modded to take context as input and perform "attention gating" in residual blocks"""
...
class MaskedResidualBlock(nn.Module):
    ...
    def forward(self, inputs, context=None):
        ...
        # NOTE: we added this to enforce gating of the context features
        if context is not None:
...
class gluMaskedResidualBlock(nn.Module):
    """A residual block containing masked linear modules."""
...
class gluMADE(nn.Module):
    ...
    def __init__(
        ...
        blocks = []
        if use_residual_blocks:
            block_constructor = gluMaskedResidualBlock
        else:
            block_constructor = MaskedFeedforwardBlock
        prev_out_degrees = self.initial_layer.degrees
        for _ in range(num_blocks):
